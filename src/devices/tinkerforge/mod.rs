...
fn do_activate_devices(
    ...
    info!("Tinkerforge tinkerforge changed");
                ...
                match paket.enumeration_type {
                    EnumerationType::Available | EnumerationType::Connected => {
                        if let Some(live_end) = registered_devices.get(&uid) {
                            ...
                            info!("New device found: {uid}");
                        }
                        let session = {
                            let new_id = session_generator;
                            session_generator += 1;
                            new_id
                        };

                        info!("Registered: {uid}");
                        if let Some(device_identifier) =
                            ParsedOrRaw::from(paket.device_identifier).parsed()
                        {
                            status_updater
                            ...
                            match device_identifier {
                                ...
                                DeviceIdentifier::MasterBrick => {
                                    register_handle(
                                        ...
                                        uid,
                                        handle_master(
                                            MasterBrick::new(uid, ipcon.clone()),
                                            status_updater.clone(),
                                        ),
                                    )
                                    .await;
                                ...
                                DeviceIdentifier::IsolatorBricklet => {
                                    register_handle(
                                        ...
                                        uid,
                                        handle_isolator(
                                            IsolatorBricklet::new(uid, ipcon.clone()),
                                            status_updater.clone(),
                                        ),
                                    )
                                    .await;
